	...
	public class NPC : Char
	{
		...
		public void FollowAssign (Char _followTarget, bool _followTargetIsPlayer, float _followFrequency, float _followDistance, float _followDistanceMax, bool _faceWhenIdle, bool _followRandomDirection)
		{
			...
			if (_followTarget == null || _followFrequency <= 0f || _followDistance <= 0f || _followDistanceMax <= 0f)
			{
				...
				if (_followTarget == null) ACDebug.LogWarning ("NPC " + name + " cannot follow because no target was set.");
				else if (_followFrequency <= 0f) ACDebug.LogWarning ("NPC " + name + " cannot follow because frequency was zero.");
				else if (_followDistance <= 0f) ACDebug.LogWarning ("NPC " + name + " cannot follow because distance was zero.");
				else if (_followDistanceMax <= 0f) ACDebug.LogWarning ("NPC " + name + " cannot follow because max distance was zero.");
				return;
			}
